...
```
log p_θ(x) = Σ_{b=1}^{B} log p_θ(x^b | x^{<b})
```

Each conditional `p_θ(x^b | x^{<b})` is modeled using discrete diffusion over a block of L' tokens, conditioned on all previous blocks.
