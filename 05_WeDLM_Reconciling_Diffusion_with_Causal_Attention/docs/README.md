...
$$
\mathcal{L}_{\text{CMLM}} = -\mathbb{E}_{x \sim p_{\text{data}}}\left[\sum_{j \in \mathcal{M}} \log P_\theta(x_j \mid x_{<j} \cap \mathcal{M}^c)\right]
$$

For left-to-right languages, the conditional independence assumption:
$$
P(x_j \mid x_{\mathcal{M}^c}) \approx P(x_j \mid x_{<j} \cap \mathcal{M}^c)
$$
...
$$
\mathcal{L}_{\text{CMLM}} = -\mathbb{E}_{x \sim \mathcal{D}} \mathbb{E}_{\mathcal{M} \sim p(\mathcal{M}|x)} \left[\sum_{j \in \mathcal{M}} \log P_\theta(x_j \mid x_{<j} \cap \mathcal{M}^c)\right]
$$
...
| Concept | Formulation |
|---------|-------------|
| Objective | $\mathcal{L} = -\sum_{j \in \mathcal{M}} \log P_\theta(x_j \mid x_{<j} \cap \mathcal{M}^c)$ |
