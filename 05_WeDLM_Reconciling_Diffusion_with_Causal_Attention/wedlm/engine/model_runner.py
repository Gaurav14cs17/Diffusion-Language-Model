...
class ModelRunner:
    ...
    def _init_model(self, hf_config):
        ...
        if use_cuda and not self.enforce_eager:
    ...
    def prepare_block_tables(self, seqs: list[Sequence]) -> torch.Tensor:
        ...
        tensor = torch.tensor(block_tables, dtype=torch.int32)
    ...
    def prepare_prefill(self, seqs: list[Sequence]):
        ...
        if use_cuda:
            input_ids = input_ids.pin_memory().cuda(non_blocking=True)
            positions = positions.pin_memory().cuda(non_blocking=True)
            cu_seqlens_q = cu_seqlens_q.pin_memory().cuda(non_blocking=True)
            cu_seqlens_k = cu_seqlens_k.pin_memory().cuda(non_blocking=True)
